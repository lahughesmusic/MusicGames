...
export default function MusicalHangman() {
    ...
    const handleInputTap = (index) => {
        if (musicalAlphabet.includes(word[index])) setActiveInputIndex(index);
    };
            ...
            {word.split('').map((letter, index) => {
                ...
                return (
                    <View key={index} style={styles.columnWrapper}>
                        {isMusical && image ? <Image source={image} style={styles.noteImage} /> : <View style={styles.noteImagePlaceholder} />}
                        {isMusical ? (
                            <TouchableOpacity style={[styles.displayInput, isActive && styles.activeInput]} onPress={() => handleInputTap(index)}>
                                <Text style={styles.displayInputText}>{guessed ? guessed.toUpperCase() : ''}</Text>
                            </TouchableOpacity>
                        ) : (
                            <View style={styles.displayInput}><Text style={styles.displayInputText}>{letter.toUpperCase()}</Text></View>
                        )}
                    </View>
                );
            })}
        ...
        <View style={styles.buttonGridWrapper}>
            ...
            <View style={styles.buttonGrid}>
                {musicalAlphabet.map(letter => (
                    <TouchableOpacity key={letter} style={styles.letterButton} onPress={() => handleLetterSelect(letter)}>
...
const styles = StyleSheet.create({
    ...
    scrollContainer: { flexGrow: 1, justifyContent: 'center', alignItems: 'center', paddingTop: 0, paddingBottom: 0 },
    ...
    displayInput: { width: 24 * scale, height: 20 * scale, borderWidth: 1.5, borderColor: '#000', borderRadius: 4 * scale, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff', marginTop: Platform.OS === 'android' ? 0 : 1 * scale },
    ...
    pickMe: { fontSize: 16 * scale, fontFamily: 'Roboto-Light', marginVertical: 6 * scale, textAlign: 'center' },
    buttonGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 4 * scale },
    letterButton: { backgroundColor: '#fff', borderWidth: 1.5, borderColor: '#888', borderRadius: 2 * scale, paddingVertical: 2 * scale, paddingHorizontal: 5 * scale, marginHorizontal: 1 * scale, marginVertical: 1 * scale },
